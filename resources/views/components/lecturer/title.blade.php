                            ...
                            <tr class="border-b">
                                ...
                                <td class="text-sm text-gray-900 font-light px-6 py-4 flex">
                                    ...
                                    <div>
                                        ...
                                        <section id="decline-title-{{ $t->student->username }}" class="fixed hidden w-full lg:w-4/12 p-3 mb-5 mx-auto my-auto left-0 dark:bg-dark top-0 shadow-lg bg-primary" style="margin: auto">
                                            <div class="w-full p-4 bg-white dark:bg-secondary rounded-lg h-full max-h-[1000px] overflow-auto">
                                                <div class="w-full flex justify-between self-center px-4">
                                                    <h1 class="text-base font-semibold text-primary dark:text-white md:text-xl">Terima bimbingan {{ $t->student->name }}</h1>
                                                    <button onclick="sembunyiPopup('decline-title-{{ $t->student->username }}')" class="text-base text-black">x</button>
                                                </div>
                                                
                                                <section id="logbook" class="pt-36 pb-32 dark:bg-slate-900">
                                                    <div class="container">        
                                                        <form action="/title/decline/{{ $t->student->id }}/{{ $t->id }}" method="POST">
                                                            @csrf
