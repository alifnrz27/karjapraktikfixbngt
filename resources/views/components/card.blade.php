<section class="py-5 rounded-lg bg-gray-100 dark:bg-dark mb-4">
    <div class="">
        <div class="flex flex-wrap">
            <div class="w-1/2 px-4 lg:w-1/4">
                <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-10 lg:mb-1 dark:bg-slate-600">
                    <div class="py-8 px-6">
                        <a href="" class="block  mb-3 font-semibold text-xl text-primary dark:text-white hover:text-primary truncate">
                            <h3 class="">Tahun Ajaran</h3>
                        </a>
                        <p class="font-medium text-base text-secondary mb-6">($blogs)</p>
                    </div>
                </div>
            </div>

            <div class="w-1/2 px-4 lg:w-1/4">
                <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-10 lg:mb-1 dark:bg-slate-600">
                    <div class="py-8 px-6">
                        <a href="" class="block  mb-3 font-semibold text-xl text-primary dark:text-white hover:text-primary truncate">
                            <h3 class="">Dosen Pembimbing</h3>
                        </a>
                        <p class="font-medium text-base text-secondary mb-6">($projects)</p>
                    </div>
                </div>
            </div>

            <div class="w-1/2 px-4 lg:w-1/4">
                <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-10 lg:mb-1 dark:bg-slate-600">
                    <div class="py-8 px-6">
                        <a href="" class="block  mb-3 font-semibold text-xl text-primary dark:text-white hover:text-primary truncate">
                            <h3 class="">Tanggal Presentasi</h3>
                        </a>
                        <p class="font-medium text-base text-secondary mb-6">($experiences)</p>
                    </div>
                </div>
            </div>

            <div class="w-1/2 px-4 lg:w-1/4">
                <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-10 lg:mb-1 dark:bg-slate-600 ">
                    <div class="py-8 px-6">
                        <a href="" class="block  mb-3 font-semibold text-xl text-primary dark:text-white hover:text-primary truncate">
                            <h3 class="">Status</h3>
                        </a>
                        <p class="font-medium text-base text-secondary mb-6">($categories)</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>