                            ...
                            <tbody>
                                ...
                                <tr class="border-b">
                                    ...
                                    <td class="text-sm text-gray-900 font-light px-6 py-4 flex">
                                        <form action="/after-presentation/accept/{{ $s->user_id }}/{{ $s->id }}" method="post">
                                            @csrf
                                            <button type="submit">
                                        ...
                                        <div>
                                            <button onclick="tampilPopup('decline-after-presentation-{{ $s->user->username }}')">
                                            ...
                                            <section id="decline-after-presentation-{{ $s->user->username }}" class="fixed hidden w-full lg:w-4/12 p-3 mb-5 mx-auto my-auto left-0 dark:bg-dark top-0 shadow-lg bg-primary" style="margin: auto">
                                                <div class="w-full p-4 bg-white dark:bg-secondary rounded-lg h-full max-h-[1000px] overflow-auto">
                                                    <div class="w-full flex justify-between self-center px-4">
                                                        ...
                                                        <button onclick="sembunyiPopup('decline-after-presentation-{{ $s->user->username }}')" class="text-base text-black">x</button>
                                                    </div>
                                                    
                                                    <section id="logbook" class="pt-36 pb-32 dark:bg-slate-900">
                                                        <div class="container">        
                                                            <form action="/after-presentation/decline/{{ $s->user_id }}/{{ $s->id }}" method="POST">
                                                                @csrf
